...
{
	IplImage* stacked = cvCreateImage(cvSize(
...
void display_big_img( IplImage* img, char* title )
{
  ...
  if( img->width >= 0.90 * scr_width  ||  img->height >= 0.90 * scr_height )
	...
	scale = 0.90 * scr_height / img->height;
      ...
      small1 = cvCreateImage( cvSize( img->width * scale, img->height * scale ),
			     img->depth, img->nChannels );
      cvResize( img, small1, CV_INTER_AREA );
    }
  else
    small1 = cvCloneImage( img );
  
  cvNamedWindow( title, 1 );
  cvShowImage( title, small1 );
